<template>
    ...
    <Hotspot-Sidebar :hotspot-info="hotspotInfo" :sub-hotspot-list="subHotspotList" :city-id="cityId" @remove-hotspot-clicked="removeHotspotClicked"></Hotspot-Sidebar>
    <main class="mdl-layout__content hotspot-content">
      <section class="section--center mdl-grid mdl-grid--no-spacing mdl-shadow--2dp" v-for="item in contentGrps">
        <header class="section__play-btn mdl-cell mdl-cell--3-col-desktop mdl-cell--2-col-tablet mdl-cell--4-col-phone mdl-color--teal-100 mdl-color-text--white">
          <i class="material-icons">play_circle_filled</i>
        </header>
        <div class="mdl-card mdl-cell mdl-cell--9-col-desktop mdl-cell--6-col-tablet mdl-cell--4-col-phone">
          <div class="mdl-card__supporting-text">
            <h4>{{item.title}} － role: {{item.roleId}}</h4>
            <p v-if="item.description">{{item.description}}</p>
            <p v-else>暂无描述</p>
          </div>
          <div class="mdl-card__actions">
            <a href="#" class="mdl-button">修改</a>
            <mdl-button>修改</mdl-button>
  ...
  <mdl-dialog title="确认" display-on="confirmAction">
    <p>确认要删除热点<strong>{{hotspotInfo.name}}</strong>吗？</p>
    <template slot="actions">
...
import GroupService from '../content/GroupService.js'
...
export default {
  ...
  route: {
    data: function (transition) {
      ...
      var hotspotInfo = HotspotService.getHotspotInfo(hotspotId)
      var subHotspotList = HotspotService.getSubHotspots(hotspotId)
      var hotspotLocations = HotspotService.getHotspotLocations(hotspotId)
      var contentGrps = GroupService.getGrpByHotspotId(hotspotId)
      // var center = [this.hotspotInfo.centerLongitude, this.hotspotInfo.centerLatitude]
      // this.$broadcast('setCenter', center)
      return {
        cityId: transition.to.params.cityId,
  ...
  methods: {
    ...
    updateHotspotEdit: function () {
      var self = this
      var promise = HotspotService.updateHotspot(this.hotspotInfo.id, this.hotspotInfo)
      promise.then(function (response) {
        // success callback
        console.log('success to update hotspot')
        self.$dispatch('successMsg', 'Update successfully!')
      }, function (response) {
        // error callback
  ...
  events: {
    'editHotspotClicked': function () {
      ...
      console.log('setRegion' + locations)
      this.$broadcast('setRegion', locations)
    },
    ...
    'removeHotspotClicked': function (id) {
      var self = this
      this.$broadcast('confirmAction', function () {
        // remove hotspot
        console.log('remove hotspot id: ' + id)
        var promise = HotspotService.removeHotspot(id)
        promise.then(function (response) {
          // success callback
          console.log('success to remove hotspot')
          self.$dispatch('successMsg', 'Remove successfully!')
        }, function (response) {
          // error callback
          self.$dispatch('errorMsg', response.data)
        })
